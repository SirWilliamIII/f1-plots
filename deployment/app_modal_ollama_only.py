...
# Create persistent volume for Ollama models (prevents re-downloading 4.7GB each time!)
...
root_dir = local_dir.parent  # Go up to project root where modelfile is
...
image = (
    ...
    .add_local_file(
        str(root_dir / "f1-analyst.modelfile"),
...
@app.function(
    ...
    volumes={"/root/.ollama": ollama_volume},  # Persist Ollama models across invocations
)
def generate(
    model: str,
    prompt: str,
    ...
    try:
        ...
        # Commit volume to persist any newly downloaded models
