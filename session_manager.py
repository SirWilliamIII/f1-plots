...
class SessionManager:
    ...
    def preload_popular_sessions(self):
        ...
        self.logger.info(f"🔄 Starting preload of {len(popular_sessions)} popular sessions")
        
        for year, race, session_type in popular_sessions:
            self.preload_executor.submit(self._preload_session, year, race, session_type)
    ...
    def _load_session_with_timeout(self, year: int, race: str, session_type: str, cache_key: str):
        """
        ...
        try:
            ...
            load_time = time.time() - start_time
            error_msg = f"Failed to load session after {load_time:.2f}s: {cache_key} - {str(e)}"
            self.logger.error(f"❌ {error_msg}")
            
            # Provide helpful error context
            if "SessionNotAvailableError" in str(e):
                raise Exception(f"Session data not available for {year} {race} {session_type}. "
                              f"Try a different year/race combination.")
            elif load_time > max_load_time:
                raise Exception(f"Session loading timed out after {max_load_time}s. "
        ...
        with self.cache_lock:
            if keep_popular:
                ...
                keys_to_remove = [
                    ...
                    if key not in popular_keys
                ]
                
                for key in keys_to_remove:
                    del self.cache[key]
                
                self.logger.info(f"🧹 Cleared {len(keys_to_remove)} non-popular sessions from cache")
            else:
