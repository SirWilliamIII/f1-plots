...
if docker-compose -f docker-compose.remote-ollama.yml ps | grep -q "Up"; then
    ...
    if curl -s http://localhost:8080/ollama_proxy/tags > /dev/null; then
        echo "✅ Remote Ollama connection successful!"
    else
        echo "⚠️  Warning: Could not connect to remote Ollama. Please check:"
        echo "   - SSH tunnel is active: ssh -i ~/.ssh/lambda-gpu.pem ubuntu@129.213.30.129"
        echo "   - Ollama is running on the remote server"
        echo "   - Port 11434 is accessible"
    fi

    echo ""
    echo "🎉 Deployment complete!"
    echo "📱 App is available at: http://localhost:8080"
    echo "🔗 Ollama endpoint: http://129.213.30.129:11434"
    echo ""
